...
// Function to calculate divided differences for forward interpolation
std::vector<std::vector<double>> calculateDividedDifferences(const std::vector<double>& x, const std::vector<double>& y) {
    ...
    // Calculate the divided differences for forward interpolation
    for (int j = 1; j < n; j++) {
        for (int i = 0; i < n - j; i++) {
...
void displayDividedDifferencesTable(const std::vector<std::vector<double>>& f) {
    ...
    std::cout << "Divided Differences Table (Forward Interpolation):\n";
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n - i; j++) {
            std::cout << f[i][j] << "\t";
        }
...
// Function to perform Newton's divided difference forward interpolation
double newtonDividedDifferenceForwardInterpolation(const std::vector<double>& x, const std::vector<double>& y, double value) {
    int n = x.size();
...
int main() {
    ...
    // Calculate the interpolated longitude using Newton's divided difference forward interpolation
    double interpolatedLongitude = newtonDividedDifferenceForwardInterpolation(x, y, latitudeToInterpolate);
